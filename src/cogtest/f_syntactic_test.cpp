...
{
public:
	FunctionalSyntacticTestFixture()
		: SyntacticTestFixture("test/functional/syntactic")
	{
		return;
	}
};

BeginSuiteFixture(FunctionalSyntacticTest, FunctionalSyntacticTestFixture);
...
Case(s2_comment_hash_code)
{
	ParseFile("comment_hash_code.cog");
	AssertResult(0, 0);
}

Case(s2_comment_hash_symbol)
{
	ParseFile("comment_hash_symbol.cog");
	AssertResult(0, 0);
}

Case(s2_comment_slash_code)
{
	ParseFile("comment_slash_code.cog");
	AssertResult(0, 0);
}

Case(s2_comment_slash_symbol)
{
...
Case(s2_label_redefinition)
{
	ParseFile("label_redefinition.cog");
	AssertResult(0, 0);
}

Case(s2_literal_float)
{
...
Case(s2_symbol_basic)
{
	ParseFile("symbol_basic.cog");
	AssertResult(0, 0);
}

Case(s2_symbol_empty)
{
...
EndSuite(FunctionalSyntacticTest);
